    ...
    with tf.Session() as sess:
        ...
        for i in range(15):
            ...
            for start, end in training_batch:
                ...
                filteredImage = sess.run(l1a, feed_dict={X: trX[0].reshape(1, 32, 32, 3),
                                                         p_keep_conv: 1.0, p_keep_hidden: 1.0})  # (1,32,32,64)
                
                print("filtered image shape:", filteredImage.shape[3])
                print(tf.size(filteredImage))
# tf.summary.image('Input Image', tf.transpose(tf.reshape(X, shape=[batch_size, 3, 32, 32]), perm=[0, 2, 3, 1]))
                filteredImage = sess.run(tf.transpose(tf.reshape(filteredImage, [1, 3, 32, 32]), (0, 2, 3, 1)))  # (64,32,32,1)
